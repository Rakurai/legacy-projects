...
| `brief` | string? | One-sentence answer to "what does this entity do or represent?" — see per-kind semantics below |
| `details` | string? | Paragraph-level description of internals — see per-kind semantics below |
| `params` | dict? | Parameter name → description mapping (functions only) |
| `returns` | string? | Return value description (functions only; omitted when void) |
| `tparams` | dict? | Template parameter name → description mapping (omitted when none) |
| `throws` | string? | Description of errors or exceptions thrown (omitted when none) |
| `notes` | string? | Interesting behaviors, design details, or usage patterns that don't fit in `brief`/`details` — see per-kind semantics below |
| `rationale` | string? | Why/when this entity would be used, inferred from call-site usage examples — see per-kind semantics below |
| **Pipeline fields** | | |
| `state` | string | Current documentation state (see lifecycle below) |
| `prompt` | string? | The LLM prompt used to generate this doc |
| `response` | DoxygenFields? | Raw LLM response (structured); cleared to `null` after `commit_docs()` merges it into the main fields |
| `usages` | dict? | Maps `"{compound_id}, {signature}"` → call-site usage description (from backward pass) — see below |

### Per-kind field semantics

The `brief`, `details`, `notes`, and `rationale` fields are generated by kind-specific Jinja2 templates (`templates/docgen_fw_refine_*.j2`). Their meaning differs by entity kind:

#### `brief`

A single sentence. The question it answers depends on kind:

| Kind(s) | What `brief` answers |
|---------|----------------------|
| `function`, `define` | What does this function do? (action-oriented, e.g. "Applies damage to the target character and updates combat state.") |
| `variable` | What does this variable represent or store? |
| `class`, `struct` | What does this type represent? |
| `enum` | What does this enumeration represent? |
| `group` | What does this group of constants represent? |
| `namespace`, `file` | What is the purpose/scope of this unit? |

#### `details`

A paragraph (two to five sentences). The angle depends on kind:

| Kind(s) | What `details` covers |
|---------|----------------------|
| `function`, `define` | How it works internally — algorithms, control flow, state it reads/mutates, interactions with callee functions |
| `variable` | How it is used, initialized, and how it relates to adjacent data structures |
| `class`, `struct` | Purpose, notable members, key methods, and behavioral contracts |
| `enum` | Purpose and description of the value set it defines |
| `group` | Purpose, how the constants are organized, and what they collectively represent |

#### `notes`

Optional. Captures information that is true and useful but doesn't belong in the main description:

- **Functions:** Unusual control flow, edge-case behavior, side effects, thread-safety caveats, or subtle invariants the caller must preserve.
- **Variables:** Storage patterns (e.g., always heap-allocated, never `null` after init), naming collisions with other unrelated variables, or iterator roles.
- **Classes/structs:** Important design patterns (e.g., intrusive list node, CRTP base, arena-allocated), lifecycle rules, or ownership semantics.
- **Enums/groups:** Usage patterns, sentinel values, or flag-combination conventions.

#### `rationale`

Optional. Answers "why does this entity exist and when would a caller use it?" Populated primarily by Phase 3 (forward refinement), using `usages` from Phase 2 as evidence. Distinct from `details` in that it focuses on motivation and call-site perspective rather than implementation:

- **Functions:** When and why another function would call this one — what problem it solves from the caller's viewpoint.
- **Variables:** What system behavior the variable enables or supports — why it is stored at all.
- **Classes/structs:** How the type fits into the broader system architecture.
- **Enums/groups:** Why the flag set or value set is organized this way and what it enables at call sites.

#### `usages`

A dict produced by the backward pass (Phase 2, `docgen_backward.ipynb`). Each key is `"{compound_id}, {signature}"` identifying the *caller* (the function that uses this entity). Each value is a natural-language sentence describing **how that specific caller uses this entity** — what it passes as arguments, what it does with the return value, and the semantic role the entity plays in that caller's logic.

Example (from `_affect_8hh.json`, variable `prev`):
```json
{
  "handler_8cc, void extract_obj(Object *obj)": "Used to traverse the object list to find and unlink the object being extracted.",
  "_note_8cc, void note_remove(Character *ch, Note *pnote, bool del)": "Used to traverse the linked list to find the node preceding 'pnote' for removal from the list."
}
```

`usages` is the raw material that `rationale` is synthesized from during Phase 4 (backward refinement). After refinement, low-plausibility or vague entries are discarded or rewritten in place. `usages` entries that survive refinement represent high-confidence, specific descriptions of real call-site behavior.
