...
def compose_codeline_reference_list(db: dp.EntityDatabase) -> Generator[Tuple[dp.EntityID, str, str], None, None]:
    """
    Compose a list of code line references from the entity database.
    ...
    for c in db.compounds.values():
        if not isinstance(c, dp.DoxygenFile):
            continue

        for source_eid, target_eids in c.codeline_refs.items():
            for target_eid in target_eids:
                yield (source_eid, target_eid, 'ref')
...
def get_code_body(root_path: Path, db: dp.EntityDatabase, g: nx.DiGraph, node_id: str) -> Dict[str, str|dp.LocationID]:
    kind = g.nodes[node_id].get("kind")

    if kind == 'group':
        # special case for groups, we need to composite the body of all members
        members = [pred for pred in g.predecessors(node_id)
                   if g.edges[pred, node_id].get("type") == Relationship.CONTAINED_BY]
        parts = [get_code_body(root_path, db, g, pred) for pred in members]
        if len(parts):
            part_loc_strings = []
            part_body_strings = []
            locs = {}
            for p in parts:
                locs.setdefault(p['loc'].fn, []).append((p['loc'].line, p['loc'].end_line, p['code']))
            for fn, spans in locs.items():
                loc_id = f"{fn}:{min(s[0] for s in spans)}-{max(s[1] for s in spans)}"
                part_loc_strings.append(loc_id)
                part_body_strings.append(''.join([s[2] for s in spans]))

            return {
                'loc': '|'.join(part_loc_strings),
                'code': ''.join(part_body_strings)
            }
        return None
...
#                           DEFINE (  83/  83)                      
#                             ENUM (  11/  11)                      
#   function ( 273/2646)  function (1870/2646)  function (1256/2646)
#                         VARIABLE (2272/2272)  variable (  98/2272)
#      class (  39/  56)     CLASS (  56/  56)                      

# EVERYTHING should have a body location, if not, we'll raise an error
# classes and structs need their body locations adjusted

    if kind in ('namespace', 'file', 'dir'):
        return None

    source_nodes = {}
    for pred in g.predecessors(node_id):
        if g.edges[pred, node_id].get("type") == Relationship.REPRESENTED_BY:
            source_nodes.setdefault(g.nodes[pred].get('type'), []).append(pred)

    if len(source_nodes.get('body', [])) == 0:
        return None
#        raise Exception(f"no body node for {node_id}")

    body_node = source_nodes['body'][0] # this should always exist
    body_loc_id = dp.LocationID.from_str(body_node)
    body = db.locations[body_loc_id]

    start = body.line
    end = body.end_line or body.line

    # if we can find a file line within a short distance above the
    # body we'll take it as the start of the class or struct definition.
    # this could be wrong if there is a very long inheritance list.
    if kind in ('class', 'struct'):
        for file_node in source_nodes.get('file', []):
            file_loc_id = dp.LocationID.from_str(file_node)
            file = db.locations[file_loc_id]
            if file.fn == body.fn and 0 < file.line - body.line < 10:
                start = file.line
                break

    from src_utils import extract_lines
    code = extract_lines(root_path / body.fn, start, end) # caches
    return {
        'loc': dp.LocationID(fn=body.fn, line=start, end_line=end),
        'code': code
    }

def build_condensation_dag(graph):
    """
    Create a condensation DAG by collapsing strongly connected components (SCCs)
    in the member-level requires subgraph.
    """
    member_nodes = [
        n for n, d in graph.nodes(data=True)]# if d.get("type") == graph_builder.MEMBER]
    required_by_edges = [
        (u, v) for u, v, d in graph.edges(data=True)
        if d.get("type") == Relationship.REQUIRED_BY and u in member_nodes and v in member_nodes
    ]

    subgraph = nx.DiGraph()
    subgraph.add_nodes_from(member_nodes)
    subgraph.add_edges_from(required_by_edges)

    sccs = list(nx.strongly_connected_components(subgraph))

    condensation = nx.condensation(subgraph, sccs)
    id_to_members = {i: list(scc) for i, scc in enumerate(sccs)}

    # Attach member node attributes to the SCC nodes in the condensation DAG
    for node_id, members in id_to_members.items():
        condensation.nodes[node_id]["members"] = members
        condensation.nodes[node_id]["size"] = len(members)
        condensation.nodes[node_id]["label"] = "\\n".join(
            graph.nodes[m].get("name", m) for m in members
        )

    # filter out single-member SCCs
    condensation = condensation.subgraph(
        n for n, d in condensation.nodes(data=True) if d.get("size", 0) > 1
    ).copy()
    print(f"Found {len(sccs)} strongly connected components")
    print(f"Remaining SCCs after filtering: {condensation.number_of_nodes()}")

    return condensation

def test_condensation_dag(graph, out_path: Path):
    import json

    condensation_dag = build_condensation_dag(graph)
    ...
    data = {
        "nodes": [
            ...
            for n, d in condensation_dag.nodes(data=True)
            if len(d.get("members")) > 1 # ignore single-member SCCs
        ],
        "edges": [
            {"from": u, "to": v} for u, v in condensation_dag.edges()
        ]
    }

    # Save the condensation DAG to a JSON file
    with open(out_path, 'w', encoding='utf-8') as f:
        json.dump(data, f, indent=2)

def fan_in(g: nx.DiGraph, node_id: str, edge_type: Relationship) -> int:
    if edge_type:
        if type(edge_type) == Relationship:
            edge_type = [edge_type]
        edge_type = set(edge_type)
    return [pred for pred in g.predecessors(node_id)
        if not edge_type or g.edges[pred, node_id].get("type") in edge_type]

def load_graph(gml_path: Path) -> nx.MultiDiGraph:
    """Load the call graph from a GML file."""
    if not gml_path.exists():
        raise FileNotFoundError(f"Graph file {gml_path} does not exist.")

    try:
        # Load as a MultiDiGraph
        g = nx.read_gml(gml_path, destringizer=int)
    except Exception as e:
        log.error(f"Failed to read GML file {gml_path}: {e}")
        raise

    # If loaded graph is not a MultiDiGraph, convert it
    if not isinstance(g, nx.MultiDiGraph):
        log.info(f"Converting loaded graph to MultiDiGraph")
        mg = nx.MultiDiGraph()
...
def save_graph(g: nx.MultiDiGraph, gml_path: Path):
    """Export the call graph to a GML file."""
