...
class GetClassHierarchyParams(BaseModel):
    ...
    entity_id: str = Field(description="Class entity ID")
...
async def get_callers_tool(
    ...
    callers_by_depth_ids = get_callers_fn(graph, params.entity_id, params.depth, params.limit)

    # Fetch entity summaries for each depth
    callers_by_depth: dict[int, list[EntitySummary]] = {}
    total_callers = 0
    ...
    for depth, entity_ids in callers_by_depth_ids.items():
        ...
        for eid in entity_ids:
            ...
            if entity:
                entities.append(entity_to_summary(entity))

        callers_by_depth[depth] = entities
        total_callers += len(entities)
...
async def get_callees_tool(
    ...
    callees_by_depth_ids = get_callees_fn(graph, params.entity_id, params.depth, params.limit)
    ...
    for depth, entity_ids in callees_by_depth_ids.items():
        ...
        callees_by_depth[depth] = entities
        total_callees += len(entities)
...
async def get_dependencies_tool(
    ...
    if params.entity_id not in graph:
    ...
    if params.direction in ("outgoing", "both"):
        for _, target, data in graph.out_edges(params.entity_id, data=True):
            ...
            entity = await session.get(Entity, target)
            if entity:
                dependencies.append({
                    "entity": entity_to_summary(entity).model_dump(),
                    "relationship": edge_type,
                    "direction": "outgoing",
                })

            if len(dependencies) >= params.limit:
                break

    # Get incoming edges
    if params.direction in ("incoming", "both") and len(dependencies) < params.limit:
        for source, _, data in graph.in_edges(params.entity_id, data=True):
            edge_type = data.get("type", "")
            if params.relationship and edge_type != params.relationship:
                continue

            entity = await session.get(Entity, source)
            if entity:
                dependencies.append({
                    "entity": entity_to_summary(entity).model_dump(),
                    "relationship": edge_type,
...
async def get_class_hierarchy_tool(
    ...
    hierarchy = get_class_hierarchy_fn(graph, params.entity_id)
    ...
    for eid in hierarchy["base_classes"]:
        entity = await session.get(Entity, eid)
        if entity:
            base_classes.append(entity_to_summary(entity))

    derived_classes = []
    for eid in hierarchy["derived_classes"]:
        entity = await session.get(Entity, eid)
        if entity:
            derived_classes.append(entity_to_summary(entity))
...
async def get_related_entities_tool(
    ...
    for _, target, data in graph.out_edges(params.entity_id, data=True):
        all_neighbors.add((target, data.get("type", "unknown"), "outgoing"))

    for source, _, data in graph.in_edges(params.entity_id, data=True):
        all_neighbors.add((source, data.get("type", "unknown"), "incoming"))

    # Group by relationship
    for neighbor_id, rel_type, direction in all_neighbors:
        if total_neighbors >= params.limit:
            break
        ...
        if entity:
            ...
            neighbors_by_relationship[key].append(entity_to_summary(entity))
            total_neighbors += 1
