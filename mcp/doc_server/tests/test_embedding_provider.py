...
class TestLocalEmbeddingProviderIntegration:
    ...
    def test_embed_returns_correct_length(self, provider):
    ...
    async def test_aembed(self, provider):
        vec = await provider.aembed("async test")
        assert len(vec) == 768

    @pytest.mark.asyncio
    async def test_aembed_batch(self, provider):
        vecs = await provider.aembed_batch(["a", "b", "c"])
        assert len(vecs) == 3
        assert all(len(v) == 768 for v in vecs)
...
class TestProtocolConformance:
    ...
    def test_local_provider_has_required_attributes(self):
        ...
        assert hasattr(LocalEmbeddingProvider, "max_batch_size")
        assert hasattr(LocalEmbeddingProvider, "embed")
        assert hasattr(LocalEmbeddingProvider, "embed_batch")
        assert hasattr(LocalEmbeddingProvider, "aembed")
        assert hasattr(LocalEmbeddingProvider, "aembed_batch")

    def test_hosted_provider_has_required_attributes(self):
        assert hasattr(HostedEmbeddingProvider, "dimension")
        assert hasattr(HostedEmbeddingProvider, "max_batch_size")
        assert hasattr(HostedEmbeddingProvider, "embed")
        assert hasattr(HostedEmbeddingProvider, "embed_batch")
        assert hasattr(HostedEmbeddingProvider, "aembed")
        assert hasattr(HostedEmbeddingProvider, "aembed_batch")
