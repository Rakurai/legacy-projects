...
from . import doxygen_graph as dg

from .subsystem_utils import NodeID, ClusterID, SubsystemID, Subsystem
...
class EmbeddingRefiner:
    ...
    def __init__(self, db: dp.EntityDatabase, graph: nx.Graph, doc_database: doc_db.DocumentDB=None):
        ...
        self.tokenizer = None
        self.model = None
        self.sbert_model = None  # legacy compat — now backed by OpenAI
    ...
    def load_sbert_model(self) -> None:
        """Initialise the embedding backend (Qwen3-Embedding-8B via LM Studio).

        Kept as `load_sbert_model` for API compatibility with existing callers.
        """
        from . import openai_embeddings as oai_emb
        self._oai_emb = oai_emb
        self._openai_ready = True
        # Provide a thin shim so callers that accessed refiner.sbert_model.encode()
        # keep working (e.g. cap_probe.py).
        self.sbert_model = self  # self acts as the model object

    # Shim: make refiner.sbert_model.encode(text, ...) work via local embedding API
    def encode(self, text_or_texts, *, normalize_embeddings=True, convert_to_numpy=True, **kw):
        """Encode one or more texts using the configured embedding model."""
        if not self._openai_ready:
            self.load_sbert_model()
        if isinstance(text_or_texts, str):
            return self._oai_emb.embed_single(text_or_texts)
        return self._oai_emb.embed_texts(list(text_or_texts))

    def get_sentence_embedding_dimension(self) -> int | None:
        """Return embedding dimensionality (None until first embed call)."""
        from . import openai_embeddings as oai_emb
        return oai_emb.DIMENSIONS
    ...
    def generate_node_embedding(self, node_id: NodeID) -> Optional[Embedding]:
        ...
        # Get entity documentation (skip nodes without doc_db entries)
        compound_docs = self.docs.docs.get(entity.id.compound)
        if compound_docs is None:
            return None
        doc = compound_docs.get(entity.signature)
        if doc is None:
            return None
    ...
    def generate_node_embedding_sbert(self, node_id: NodeID) -> Optional[Embedding]:
        """Generate an embedding vector for a given node using the configured embedding model.

        Kept as `generate_node_embedding_sbert` for API compatibility.
        ...
        return self._oai_emb.embed_single(docstring)
        ...
        for subsystem_name, sim_data in similarity_matrices.items():
            ...
            for i, node_id in enumerate(node_ids):
                ...
                if avg_similarity < threshold:
                    entity = su.get_entity(self.db, node_id)
                    outlier_info = {
                        'node_id': node_id,
                        'name': entity.name if entity else node_id[:16],
...
#                    'confidence': float(best_similarity - outlier['average_similarity']),
    ...
    def refine_subsystems(self, subsystems: Dict[SubsystemID, Subsystem], embeddings: Dict[SubsystemID, Dict[NodeID, Embedding]]) -> Dict:
        ...
        seen_nodes: set = set()  # prevent double-moves for the same node
        for suggestion in suggestions:
            if suggestion['confidence'] > CONFIDENCE_THRESHOLD:
                ...
                # Skip if node was already moved, or subsystem IDs are invalid
                if node_id in seen_nodes:
                    continue
                if current not in refined_subsystems or target not in refined_subsystems:
                    continue
                if node_id not in refined_subsystems[current].nodes:
                    continue

                # Move node to suggested subsystem
                refined_subsystems[current].nodes.discard(node_id)
