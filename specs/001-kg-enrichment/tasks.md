...
> Generated by `/speckit.audit` on 2026-03-19. Address before next `/speckit.implement` run.

- [X] T029 [AR] Raise `ValueError` for invalid `source` parameter in `server/tools/search.py:53-60` — replace the `log.warning` + empty-response return with a `ValueError("Unsupported search source: {source}")`. Update the existing test `test_search_tool_unsupported_source` in `tests/test_search_tool.py` to assert `pytest.raises(ValueError)` instead of checking empty results. (SF-001)

- [X] T030 [AR] Remove embedding-failure silent fallback in `server/search.py:143-147` (`hybrid_search`) — let `(OSError, RuntimeError, TimeoutError)` propagate instead of catching and degrading to keyword-only. The caller (search tool) already receives `embedding_provider=None` when no provider is configured, which triggers the legitimate keyword-only path. Runtime embedding failures are a real error that should surface. Remove the `search_mode = SearchMode.KEYWORD_FALLBACK` reassignment in the except branch. (CV-001)

- [X] T031 [AR] Remove same embedding-failure silent fallback in `server/search.py:219-223` (`hybrid_search_usages`) — same fix as T030 applied to the usages search path. Let embedding errors propagate. (CV-002)

- [X] T032 [P] [AR] Extract the fan_in ranking subquery (duplicated ~25 lines) from `server/tools/explain.py:59-82` and `server/tools/entity.py:101-125` into a shared async helper `fetch_top_callers` in `server/util.py`. Both call sites delegate to this helper. Moved `from sqlalchemy import outerjoin` to module-level imports. (CQ-001, CQ-002)

- [X] T033 [P] [AR] Add `SearchSource` StrEnum to `server/enums.py` with members `ENTITY = "entity"` and `USAGES = "usages"`. Updated `source` parameter type in `server/tools/search.py` search function from `str` to `SearchSource`. Updated test `test_search_tool_unsupported_source` to assert `pytest.raises(ValueError)`. (SD-001)

- [X] T034 [P] [AR] Remove silent catch in `server/tools/entity.py:173-174` (`get_source_code` context line reads) — let `OSError`/`UnicodeDecodeError` propagate. Updated `test_get_source_code` to use `context_lines=0` (test env has no source files on disk). (CV-003)

- [X] T035 [AR] Remove embedding-failure silent fallback in `server/resolver.py:275-276` — same pre-existing pattern as CV-001/CV-002 in the resolution pipeline. Let embedding errors propagate instead of logging and continuing. Updated `test_resolve_semantic_stage_handles_client_error` → `test_resolve_semantic_stage_propagates_client_error` to assert `pytest.raises(RuntimeError)`. (CV-001 pre-existing)

- [X] T036 Run `uv run pytest tests/ -v`, `uv run ruff check .`, `uv run mypy server/` from `mcp/doc_server/` — 187/187 tests pass, ruff clean, mypy errors all pre-existing.
