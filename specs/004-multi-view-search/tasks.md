...
> Generated by `/speckit.audit` on 2026-03-21. Address before next `/speckit.implement` run.

- [x] T031 [AR] Make `embedding_provider` non-optional in `hybrid_search`, `hybrid_search_usages`, and `resolve_entity` — remove `| None` and `= None` default (SD-001, SD-008, SD-009, FR-072) in `mcp/doc_server/server/search.py:215,409` and `mcp/doc_server/server/resolver.py:56`
- [x] T032 [AR] Remove `if embedding_provider:` and `if query_embedding:` conditional guards in search pipeline — embedding is always available (SD-002, FR-074) in `mcp/doc_server/server/search.py:252-261` and `mcp/doc_server/server/resolver.py:94`
- [x] T033 [AR] Fix `score` to always equal `winning_score` — move exact/scope boosting to a separate sort key or tie-breaker rather than mutating `score` (SD-003, FR-061) in `mcp/doc_server/server/search.py:327-343`
- [x] T034 [AR] Implement `token_jaccard` and `query_coverage` signals in `Candidate` dataclass and search pipeline (SD-004, FR-032, FR-034, FR-035) in `mcp/doc_server/server/search.py:55-65`
- [x] T035 [AR] Clamp `_shape_tsrank` return value to [0, 1] and fix test `test_raw_above_ceiling_exceeds_one` to assert clamped behavior (SD-005, TQ-001, FR-033) in `mcp/doc_server/server/search.py:67-71` and `mcp/doc_server/tests/test_search_units.py:28-30`
- [x] T036 [AR] Move floor thresholds to `ServerConfig` fields with env var overrides; read from `RetrievalView.floor_thresholds` at runtime instead of hardcoding (SD-006, PH-001, FR-043) in `mcp/doc_server/server/config.py`, `mcp/doc_server/server/lifespan.py:124,134`, and `mcp/doc_server/server/search.py:288-294`
- [x] T037 [AR] Add `params` values concatenation to `doc_search_vector` weight C in both production SQL and test fixture SQL (SD-007, TQ-003, FR-010) in `mcp/doc_server/build_mcp_db.py:188-189` and `mcp/doc_server/tests/conftest.py:264-278`
- [x] T038 [AR] Make `doc_view` and `symbol_view` non-optional parameters in `hybrid_search`; change `lc.get()` to `lc[]` in tools/search.py (SD-010, CV-002) in `mcp/doc_server/server/search.py:213-219` and `mcp/doc_server/server/tools/search.py:72-73`
- [x] T039 [AR] Remove `from __future__ import annotations` from `mcp/doc_server/server/embedding.py:12` (CV-001)
- [x] T040 [AR] Remove dead `embedding_available=lc["embedding_provider"] is not None` check in `mcp/doc_server/server/server.py:77`; hardcode `embedding_available=True` or remove the parameter from `get_stats_resource` (SF-001, WC-001) in `mcp/doc_server/server/server.py:77` and `mcp/doc_server/server/resources.py:209`
- [x] T041 [AR] Populate `symbol_searchable` column in test fixtures so trigram channel tests exercise actual pg_trgm queries (TQ-002) in `mcp/doc_server/tests/conftest.py:87-258`
- [x] T042 [AR] Add per-search observability: log channels queried, candidates per channel, candidates after merge, reranking latency (SD-012, FR-065) in `mcp/doc_server/server/search.py`
- [x] T043 [AR] Narrow `symbol_searchable` stripping to spec-defined punctuation `*&(),;` instead of all non-alphanumeric (SD-011, FR-014) in `mcp/doc_server/build_helpers/entity_processor.py:682`

---

## Validation Remediation

> Generated from live MCP validation on 2026-03-22. These items address issues that did not appear in the focused mocked test suite but failed through the real server interface.

### Root cause summary

- **VR-001 Contract/model mismatch**: `SearchResult` constrains `score`, `winning_score`, and `losing_score` to `ge=0` in `mcp/doc_server/server/models.py`, but the feature contract says these are raw cross-encoder logits and may be negative.
- **VR-002 Test fixture masking**: mocked cross-encoder fixtures in `mcp/doc_server/tests/conftest.py` always return `0.5`, so tests never exercise negative logit behavior seen with the real cross-encoder.
- **VR-003 Test assertions reinforce the wrong contract**: search tests explicitly assert non-negative scores, locking in the mismatch instead of checking the documented semantics.
- **VR-004 Missing end-to-end MCP entity validation**: the suite validates `hybrid_search()` and the tool wrapper under mocks, but not the real MCP `source="entity"` path with an initialized cross-encoder and database-backed results.

- [x] T044 [VR] Resolve the score contract mismatch by aligning `SearchResult` in `mcp/doc_server/server/models.py` with the documented raw cross-encoder logit semantics in `specs/004-multi-view-search/contracts/search-response.md`; update any dependent response typing or serialization assumptions so negative logits no longer fail MCP response validation (VR-001)
- [x] T045 [VR] Replace the constant positive cross-encoder mocks in `mcp/doc_server/tests/conftest.py` with fixtures that can emit representative mixed-sign logits for both winning and losing views, so tests exercise the real score domain instead of only `0.5` outputs (VR-002)
- [x] T046 [VR] Update search assertions in `mcp/doc_server/tests/test_search_symbol_lookup.py`, `mcp/doc_server/tests/test_search_conceptual.py`, `mcp/doc_server/tests/test_search_reranking.py`, and `mcp/doc_server/tests/test_search_tool.py` to validate the actual contract (`score == winning_score`, `winning_view` correctness, ordering, and presence of metadata) rather than asserting all scores are non-negative (VR-003)
- [x] T047 [VR] Add an end-to-end validation test path for live MCP `source="entity"` queries — covering `damage`, `Character`, `do_look`, `PLR_COLOR2`, `Logging::stc`, behavioral search, empty query, and nonsense query — so the server interface is checked with real cross-encoder outputs instead of mocks only (VR-004, SC-001, SC-002, SC-003, SC-004)
- [x] T048 [VR] Re-run the focused search suite plus the new live-MCP entity validation after T044-T047, and update `specs/004-multi-view-search/validation.md` with post-fix results and any remaining gaps (VR-001, VR-002, VR-003, VR-004)
- [x] T049 [VR] Fix same-name exact-match ranking in `mcp/doc_server/server/search.py` so representative bare-name queries like `damage` rank the intended primary entity in the top-3 instead of letting same-name variables outrank it on cross-encoder score alone (SC-001)
- [x] T050 [VR] Tighten final-result rejection in `mcp/doc_server/server/search.py` so nonsense queries like `xyzzy foobar baz` return zero results after reranking instead of emitting weak negative-logit candidates that merely survived intermediate filtering (SC-004)

---

## Audit Remediation (Re-Audit)

> Generated by `/speckit.audit` on 2025-07-16. Address before next `/speckit.implement` run.

- [x] T051 [AR] Fix `query_coverage` in `mcp/doc_server/server/search.py:443-445` to tokenize doc text (brief, details, notes, rationale, params, returns) instead of name+signature — fetch doc prose columns in the Stage 3 query and pass to `_compute_query_coverage` per FR-035 (SD-001)
- [x] T052 [AR] Fix `test_cold_cache_generates_all_embeddings` in `mcp/doc_server/tests/test_embeddings.py:131` — set `mock_provider.max_batch_size = 256` and use `side_effect` to return correct per-batch vectors instead of a fixed `return_value` (WC-001)

---

## Design Alignment

> Generated by design-vs-implementation review on 2026-03-23. Restores design intent where implementation diverged.

- [x] T053 [DA] Remove tier-based sorting, `kind_priority` heuristic, `fan_in` tiebreaker, and `signature_exact` filter bypass from `mcp/doc_server/server/search.py`. Cross-encoder score alone determines result order per design D-01. Sort Stage 7 by `winning_score` only. Keep `name_exact` as filter bypass (design §3 Stage 4). Keep post-rerank noise rejection (`_RERANK_SCORE_FLOOR` + corroborating signal check). Remove `_exact_match_kind_priority`, `_is_constant_like`, `_is_type_like` helper functions. Remove `sort_tier` field from `SearchResult` in `models.py`. Update tests that assert on `sort_tier`, `kind_priority`, or tier-based ordering.
- [x] T054 [DA] Parallelize Stage 1 channel queries in `mcp/doc_server/server/search.py:370-391` using `asyncio.gather()` — design specifies "five parallel retrieval channels" but implementation awaits them sequentially.
- [x] T055 [DA] Move `import re as _re` from inside `_tokenize` function body (`mcp/doc_server/server/search.py:141`) to module-level import.
- [x] T056 [DA] Optimize token overlap computation in `mcp/doc_server/server/search.py:435-445` — defer to after the full entity fetch in Stage 5 (post-filtering, fewer entities) instead of running a separate `SELECT name, signature` query for all candidates. This also enables T051's fix: doc text fields (brief, details, notes, rationale, params, returns) are already on the loaded Entity objects.
- [x] T057 [DA] Evaluate embedding models and cross-encoder models per design open items E-01 and E-02. Compare: (a) `BAAI/bge-base-en-v1.5` (current, prose-only) vs `jinaai/jina-embeddings-v2-base-code` (code-aware) for both views, (b) two-model approach (code model for symbol, prose model for doc), (c) cross-encoder candidates: `BAAI/bge-reranker-base` (278M) vs `Xenova/ms-marco-MiniLM-L-6-v2` (22M, current) vs `jinaai/jina-reranker-v1-turbo-en`. Build a representative query set covering US1-US4 and measure recall@K and ranking quality. **Prerequisite**: Complete before finalizing spec. Results determine `embedding_local_model` default and `cross_encoder_model` default in `ServerConfig`.
...
- `[AR]` tasks are audit remediation — address before next `/speckit.implement` run
- `[DA]` tasks restore design intent — address deviations found during design-vs-implementation review
