...
- [ ] T001 Create deterministic ID generator in `mcp/doc_server/build_helpers/entity_ids.py`
...
- [ ] T002 Add unit tests for entity ID generation in `mcp/doc_server/tests/test_entity_ids.py`
...
- [ ] T003 Remove `DocQuality` and `DocState` enums from `mcp/doc_server/server/enums.py` (FR-007)

- [ ] T004 Update DB model in `mcp/doc_server/server/db_models.py`
...
- [ ] T005 [P] Update API response models in `mcp/doc_server/server/models.py`
...
- [ ] T006 [P] Update converters in `mcp/doc_server/server/converters.py`
...
- [ ] T007 [P] Remove `resolve_entity_id` helper from `mcp/doc_server/server/util.py` (FR-011)
...
- [ ] T008 [US1] Integrate deterministic IDs into entity processing in `mcp/doc_server/build_helpers/entity_processor.py`
...
- [ ] T009 [US1] Update graph loader for ID translation in `mcp/doc_server/build_helpers/graph_loader.py`
...
- [ ] T010 [US1] Update embeddings loader for new IDs in `mcp/doc_server/build_helpers/embeddings_loader.py`
  - Re-key embedding entries by new deterministic IDs instead of old Doxygen IDs

- [ ] T011 [US1] Update build orchestrator in `mcp/doc_server/build_mcp_db.py`
...
- [ ] T013 [US1] Update test fixtures for new ID format in `mcp/doc_server/tests/conftest.py`
...
- [ ] T014 [US1] Update entity tool tests in `mcp/doc_server/tests/test_entity_tools.py`
  - Remove `resolve_entity` test cases
  - Remove tests that pass `signature` parameter
  - Update remaining tests to use new ID format

- [ ] T015 [US1] Update graph tool tests in `mcp/doc_server/tests/test_graph_tools.py`
  - Update fixtures to use `{prefix}:{7 hex}` IDs
  - Remove tests that pass `signature` parameter

- [ ] T016 [P] [US1] Update behavior tool tests in `mcp/doc_server/tests/test_behavior_tools.py`
  - Update fixtures to use `{prefix}:{7 hex}` IDs
  - Remove tests that pass `signature` parameter
...
- [ ] T017 [US2] Ensure documentation field carry-through in `mcp/doc_server/build_helpers/entity_processor.py`
...
- [ ] T018 [US2] Update converter tests in `mcp/doc_server/tests/test_converters.py`
  - Remove assertions for `doc_quality` and `doc_state` fields
  - Verify documentation fields are carried through conversion

- [ ] T019 [P] [US2] Update search tests in `mcp/doc_server/tests/test_search.py`
  - Remove `doc_quality` filter tests
  - Update any fixtures referencing removed fields

- [ ] T020 [P] [US2] Update resolution tests in `mcp/doc_server/tests/test_resolution.py` and `mcp/doc_server/tests/test_resolver_stages.py`
...
- [ ] T021 [US3] Remove `resolve_entity` tool from `mcp/doc_server/server/tools/entity.py` (FR-011)
...
- [ ] T022 [P] [US3] Remove signature parameter from graph tools in `mcp/doc_server/server/tools/graph.py` (FR-012)
  - Update `get_callers`, `get_callees`, `get_dependencies`, `get_class_hierarchy`, `get_related_entities` to accept only `entity_id`

- [ ] T023 [P] [US3] Remove signature parameter from behavior tools in `mcp/doc_server/server/tools/behavior.py` (FR-012, FR-016)
...
- [ ] T024 [P] [US3] Remove `min_doc_quality` parameter from search tool in `mcp/doc_server/server/tools/search.py` (FR-014)

- [ ] T025 [P] [US3] Remove `doc_quality_dist` from capability tools in `mcp/doc_server/server/tools/capability.py` (FR-012, FR-015)
...
- [ ] T026 [US3] Update search implementation in `mcp/doc_server/server/search.py`
...
- [ ] T027 [US3] Update resolver in `mcp/doc_server/server/resolver.py`
  - Retain resolution pipeline for search's internal use
  - Remove any signature-based entity lookup paths

- [ ] T028 [US3] Update resources in `mcp/doc_server/server/resources.py`
  - Remove `doc_quality_dist` from resource responses

- [ ] T029 [US3] Update prompts in `mcp/doc_server/server/prompts.py` (FR-017)
...
- [ ] T030 [US3] Update search tool test in `mcp/doc_server/tests/test_search_tool.py`
  - Remove `min_doc_quality` parameter test
  - Update test expectations

- [ ] T031 [P] [US3] Update capability tool tests in `mcp/doc_server/tests/test_capability_tools.py`
  - Remove `doc_quality_dist` assertions from responses

- [ ] T032 [P] [US3] Update resource tests in `mcp/doc_server/tests/test_resources.py`
...
- [ ] T033 Update util tests in `mcp/doc_server/tests/test_util.py`
  - Remove `resolve_entity_id` helper tests

- [ ] T033a Review `mcp/doc_server/tests/test_load_graph.py` and `mcp/doc_server/tests/test_graph.py`
...
- [ ] T034 Update server test in `mcp/doc_server/tests/test_server.py`
