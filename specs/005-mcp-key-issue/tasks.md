...
> Generated by `/speckit.audit` on 2026-03-17. Address before next `/speckit.implement` run.

- [X] T038 [AR] Fix stale `EntitySummary.entity_id` field description in `mcp/doc_server/server/models.py:30` — change "Internal Doxygen ID" to reflect deterministic `{prefix}:{7 hex}` format (SD-001)
- [X] T039 [AR] Remove dead `build_id_map()` function from `mcp/doc_server/build_helpers/entity_ids.py:50-95` and its tests in `test_entity_ids.py` (OE-001)
- [X] T040 [AR] Make index creation failures fatal in `mcp/doc_server/build_mcp_db.py:108-113` — remove try/except or re-raise after logging (SF-001)
- [X] T041 [AR] Narrow broad `except Exception` in `mcp/doc_server/server/resolver.py:283` to specific expected exceptions (e.g., embedding provider errors) and re-raise unexpected ones (SF-002)
- [X] T042 [AR] Narrow broad `except Exception` in `mcp/doc_server/server/search.py:155-157` to specific expected exceptions and re-raise unexpected ones (SF-003)
- [X] T043 [AR] Add `embedding: list[float] | None = None` attribute to `MergedEntity.__init__` in `mcp/doc_server/build_helpers/entity_processor.py` and remove `hasattr` guard in `mcp/doc_server/build_mcp_db.py:154` (CQ-001)
- [X] T044 [AR] Remove `from __future__ import annotations` from `mcp/doc_server/server/resolver.py:17` (CV-001)
- [X] T045 [AR] Remove `from __future__ import annotations` from `mcp/doc_server/server/search.py:19` (CV-002)
- [X] T046 [AR] Update stale module docstring in `mcp/doc_server/build_helpers/loaders.py:7` — remove reference to `entity_ids.SignatureMap` (CQ-002)
- [X] T047 [AR] Clean up comment phrasing in `mcp/doc_server/build_helpers/entity_processor.py:57` (CQ-003)